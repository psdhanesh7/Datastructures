...
//	cout << root->data << endl;
	...
	while(i < root->numChildren()) {
		TreeNode<int>* currChild = root->getChild(i);
		if(currChild->numChildren() != 0) {
			removeLeafNodes(currChild);
		}
		else {
			root->removeChild(i);
			continue;
...
/*
	for(int i = 0; i < root->numChildren(); i++) {
		...
		cout << currChild->data << endl;

		if(currChild->numChildren() != 0) {
			TreeNode<int>* newRoot = removeLeafNodes(currChild);
		}
		else {
			cout << "removed " << currChild->data << endl;
			root->removeChild(i);
		}
	}
